...
export class Connection {
  constructor(public ctx, public app_port:number, public app_id:string) {
  }
  app_ws:AppWebsocket
  session:Session
  sessions:HoloHash[]
  ...
  async open(default_content:Content, applyDeltaFn:apply_delta_fn_T) {
    const self = this
    this.app_ws = await AppWebsocket.connect(
      `ws://localhost:${this.app_port}`,
      30000,
    ...
    this.syn = new Syn(this.ctx, default_content, applyDeltaFn, this.app_ws, this.app_id)
  ...
  async joinSession() {
    ...
    if (this.sessions.length == 0) {
      ...
      if (this.session._scribe_str != this.syn.me) {
