...
import { amIScribe, selectCurrentSessionIndex, selectSessionWorkspace, } from '../../../state/selectors';
...
export function folkRequestChange(workspace, sessionHash, deltas) {
    workspace.store.update(state => {
        ...
        for (const delta of deltas) {
            sessionWorkspace.currentContent = workspace.applyDeltaFn(sessionWorkspace.currentContent, delta);
        }
...
export async function checkRequestedChanges(workspace, sessionHash) {
    ...
    if (session.requestedChanges.length > 0 &&
        ...
        session = selectSessionWorkspace(state, sessionHash);
...
export function handleChangeNotice(workspace, sessionHash, changes) {
    workspace.store.update(state => {
        ...
        if (myChanges &&
            ...
            contentToApplyTo = (_a = session.prerequestContent) === null || _a === void 0 ? void 0 : _a.content;
        }
...
function clearRequested(session, myChanges, newContent) {
