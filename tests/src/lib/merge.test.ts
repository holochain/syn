...
test(
  'check that the state of disconnected agents making changes converges after connecting',
  { concurrency: 0 },
  async t => {
    await runScenario(async scenario => {
      ...
      let aliceRootStore = await aliceSyn.createDeterministicRoot(
        sampleGrammar
      );
      ...
      let aliceWorkspaceStore = await aliceRootStore.joinWorkspace(
        workspaceHash
      );
