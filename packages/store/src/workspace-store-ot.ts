...
import { OTSessionStore } from './session-store-ot.js';
import { stateFromCommitOT, stateFromDocumentOT } from './syn-store-ot.js';

export class OTWorkspaceStore<S, E> {
  constructor(
    public documentStore: OTDocumentStore<S, E>,
  ...
  private _session: Writable<OTSessionStore<S, E> | undefined> =
    writable(undefined);
  ...
  latestSnapshot: AsyncReadable<S> = pipe(this.tip, commit =>
    commit
      ? (stateFromCommitOT(commit.entry) as S)
      : pipe(
        this.documentStore.record,
        document => stateFromDocumentOT(document.entry) as S
      )
  );
  ...
  async joinSession(
    ...
    const session = await OTSessionStore.joinSession(
      this,
